...
import 'package:risk_surveyor_app/screens/svy/survey_ext_vin.dart';
import 'package:risk_surveyor_app/screens/svy/survey_tw_vin.dart';
...
class SurveyTwExtSpd extends StatefulWidget {
  const SurveyTwExtSpd({Key? key}) : super(key: key);

  @override
  State<SurveyTwExtSpd> createState() => _SurveyTwExtSpdState();
}

class _SurveyTwExtSpdState extends State<SurveyTwExtSpd> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_ext_spd_")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_ext_spd_${imageFileList?.length}.jpg';
        ...
        child: Column(
          children: [
            Text(
              ...
              'Berdiri di depan motor sperti di gambar.'
                  '\n'
            ...
            Image.asset('assets/images/odometer.png', height: 35.h),
          ],
        ),
      );
    }
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                Get.to(const SurveyTwExtVin(), arguments: [data, surveyResult]);
              },
              heroTag: 'next',
              tooltip: 'Next',
