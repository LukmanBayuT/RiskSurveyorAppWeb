...
import 'package:risk_surveyor_app/screens/svy/survey_ext_eng_no.dart';
...
class SurveyExtVin extends StatefulWidget {
  const SurveyExtVin({Key? key}) : super(key: key);
  ...
  @override
  State<SurveyExtVin> createState() => _SurveyExtVinState();
}

class _SurveyExtVinState extends State<SurveyExtVin> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_ext_vin")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_ext_vin_${imageFileList?.length}.jpg';
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                next();
                //Get.to(const SurveyExtEngNo(), arguments: data);
              },
              heroTag: 'next',
              tooltip: 'Next',
  ...
  next() async {
    ...
    if (imageFileList!.isNotEmpty) {
      ...
      Get.to(const SurveyExtEngNo(), arguments: [data, surveyResult]);
    } else {
