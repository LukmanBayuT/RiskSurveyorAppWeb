...
import 'package:risk_surveyor_app/screens/svy/survey_checklist.dart';

class SurveyInterior extends StatefulWidget {
  const SurveyInterior({Key? key}) : super(key: key);

  @override
  SurveyInteriorState createState() {
    return SurveyInteriorState();
  }
}

class SurveyInteriorState extends State<SurveyInterior> {
  final IntListController intListController = Get.put(IntListController());
  ...
  Future<void> next(BuildContext context) async {
    //if (file.length > 0) {
    if (file.length == intListController.count && !file.containsValue("null")) {
      Get.to(SurveyChecklist());
    } else {
...
class Provider extends GetConnect {
  Future<List<dynamic>> getClaimDocList() async {
    ...
    if (true == true) {
      return [
        {"id": 111, "name": "Interior Section", "status": "Assigned"},
        {"id": 111, "name": "Karpet Kaki (Cek bagian dibawah jok, pintu bagian dalam dekat engsel yang seringnya tertutup disela-sela baut dan bawah apakah ada kotoran. Korosi dan karat yang banyak dan tidak umum)", "status": "Assigned"},
        {"id": 111, "name": "Karat (Cek bagian dibawah jok, pintu bagian dalam dekat engsel yang seringnya tertutup disela-sela baut dan bawah apakah ada kotoran. Korosi dan karat yang banyak dan tidak umum)", "status": "Assigned"},
        {"id": 111, "name": "Elektrikal (Power window, buka tutup pintu elektrik, lock pintu mobil seluruh bagian mobil)", "status": "Assigned"},
        {"id": 111, "name": "Interior Non-Standard Equipment", "status": "Assigned"},
      ];
    } else {
...
class IntListController extends GetxController with StateMixin<List<dynamic>> {
  int count = 0;
