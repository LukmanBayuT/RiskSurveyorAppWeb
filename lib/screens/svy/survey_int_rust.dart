...
import 'package:risk_surveyor_app/screens/svy/survey_int_electrical.dart';
...
class SurveyIntRust extends StatefulWidget {
  const SurveyIntRust({Key? key}) : super(key: key);
  ...
  @override
  State<SurveyIntRust> createState() => _SurveyIntRustState();
}

class _SurveyIntRustState extends State<SurveyIntRust> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_int_rust_")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_int_rust_${imageFileList?.length}.jpg';
                  ...
                  child: Card(
                    ...
                    child: Column(
                      children: [
                        ListTile(
                          title: Text(
                            "Karat (Cek bagian dibawah jok, pintu bagian dalam dekat engsel yang seringnya tertutup disela-sela baut dan bawah apakah ada kotoran. Korosi dan karat yang banyak dan tidak umum)",
                            style: TextStyle(
                                    ...
                                    children: [
                                      ListTile(
                                        ...
                                        subtitle: FormBuilderCheckboxGroup<dynamic>(
                                          ...
                                          name: 'Apakah unit kendaraan terdapat indikasi bekas terdampak banjir?',
                                          activeColor: AppColors.oonaPurple,
                                          initialValue: surveyResult.checklist!['Apakah unit kendaraan terdapat indikasi bekas terdampak banjir?'],
                                          options: const [
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                next();
                //Get.to(const SurveyIntElectrical(), arguments: data);
              },
              heroTag: 'next',
              tooltip: 'Next',
  ...
  next() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      if (imageFileList!.isNotEmpty) {
        ...
        Get.to(const SurveyIntElectrical(), arguments: [data, surveyResult]);
      } else {
