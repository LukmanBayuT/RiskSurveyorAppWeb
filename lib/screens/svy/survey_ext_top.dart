...
import 'package:risk_surveyor_app/screens/svy/survey_ext_bot.dart';
...
class SurveyExtTop extends StatefulWidget {
  const SurveyExtTop({Key? key}) : super(key: key);
  ...
  @override
  State<SurveyExtTop> createState() => _SurveyExtTopState();
}

class _SurveyExtTopState extends State<SurveyExtTop> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_ext_top_")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_ext_top_${imageFileList?.length}.jpg';
      ...
      return Center(
        child: Column(
          children: [
            ...
            Image.asset('assets/images/top.png', height: 15.h),
          ],
        ),
      );
    }
                                    ...
                                    children: [
                                      ListTile(
                                        ...
                                        subtitle: FormBuilderCheckboxGroup<dynamic>(
                                          ...
                                          name: 'Roof/Kap Atas Luar',
                                          activeColor: AppColors.oonaPurple,
                                          initialValue: surveyResult.checklist!['Roof/Kap Atas Luar'],
                                          options: const [
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                next();
                //Get.to(const SurveyExtBot(), arguments: data);
              },
              heroTag: 'next',
              tooltip: 'Next',
  ...
  next() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      if (imageFileList!.isNotEmpty) {
        ...
        Get.to(const SurveyExtBot(), arguments: [data, surveyResult]);
      } else {
