...
import 'package:risk_surveyor_app/screens/svy/survey_ext_engine_trunk.dart';
...
class SurveyExtBot extends StatefulWidget {
  const SurveyExtBot({Key? key}) : super(key: key);
  ...
  @override
  State<SurveyExtBot> createState() => _SurveyExtBotState();
}

class _SurveyExtBotState extends State<SurveyExtBot> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_ext_bot_")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_ext_bot_${imageFileList?.length}.jpg';
                                      ...
                                      children: [
                                        ListTile(
                                          ...
                                          subtitle: FormBuilderCheckboxGroup<dynamic>(
                                            ...
                                            name: 'Bagian Bawah dan Kolong',
                                            activeColor: AppColors.oonaPurple,
                                            initialValue: surveyResult.checklist!['Bagian Bawah dan Kolong'],
                                            options: const [
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                next();
                //Get.to(const SurveyExtEngTru(), arguments: data);
              },
              heroTag: 'next',
              tooltip: 'Next',
  ...
  next() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      if (imageFileList!.isNotEmpty) {
        ...
        Get.to(const SurveyExtEngTru(), arguments: [data, surveyResult]);
      } else {
