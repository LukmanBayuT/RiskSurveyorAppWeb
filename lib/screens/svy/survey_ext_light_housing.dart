...
class SurveyExtLightHousing extends StatefulWidget {
  const SurveyExtLightHousing({Key? key}) : super(key: key);
  ...
  @override
  State<SurveyExtLightHousing> createState() => _SurveyExtLightHousingState();
}

class _SurveyExtLightHousingState extends State<SurveyExtLightHousing> {
  ...
  Future<void> GetData() async {
    ...
    for (File element in filelist) {
      if (element.toString().contains("_ext_light_housing_")) {
  ...
  void selectImages() async {
    ...
    selectedImages.forEach((selectedImage) async {
      if (await Util().isImageAllowed(selectedImage)) {
        ...
        final String fileName = '${data['QuotationNo']}_ext_light_housing_${imageFileList?.length}.jpg';
        ...
        btn: [
          ...
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                next();
                //Get.to(const SurveyExtVin(), arguments: data);
              },
              heroTag: 'next',
              tooltip: 'Next',
  ...
  next() async {
    //if (_formKey.currentState?.saveAndValidate() ?? false) {
      if (imageFileList!.isNotEmpty) {
        ...
        Get.to(const SurveyExtVin(), arguments: [data, surveyResult]);
      } else {
